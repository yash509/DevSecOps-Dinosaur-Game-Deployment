pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash509/DevSecOps-Dinosaur-Game-Deployment.git'
            }
        }
        ...
        stage("Docker Image Building"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){   
                            sh "docker build -t dinosaur-game ." 
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag dinosaur-game yash5090/dinosaur-game:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/dinosaur-game:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/dinosaur-game:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/dinosaur-game:latest'
                       sh 'docker-scout cves yash5090/dinosaur-game:latest'
                       sh 'docker-scout recommendations yash5090/dinosaur-game:latest'
                       sh 'docker-scout attestation yash5090/dinosaur-game:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/dinosaur-game:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop dinosaur-game | true"
                sh "docker rm dinosaur-game | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name chatgpt-app -p 5000:80 yash5090/dinosaur-game:latest' 
                //}
            }
        }
